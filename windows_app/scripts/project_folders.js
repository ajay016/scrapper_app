...
(function (root, factory) {
    if (typeof module === 'object' && module.exports) {
        ...
        root.__pages['project_folders'] = factory();
    }
}(this, function () {
    let currentProjectId = null;
    async function fetchProjectFolders(projectId) {
        const container = document.querySelector(".project-folders-container");
        if (!container) return;
        ...
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/api/project/${projectId}/folders/`, {
                headers: {
    ...
    async function deleteSelectedFolders() {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/delete-folders/?project_id=${currentProjectId}`, {
                method: 'DELETE',
                headers: {
            ...
            if (response.ok) {
                ...
                setTimeout(() => fetchProjectFolders(currentProjectId), 1000); // Refresh
            } else {
                const data = await response.json();
    ...
    function init(params = {}) {
        if (!params.projectId) {
            console.error("projectId is required to load project folders");
            return;
        }
        currentProjectId = params.projectId;
        fetchProjectFolders(params.projectId);
