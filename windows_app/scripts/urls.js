...
(function () {
  ...
  let isPaused = false;
  let isWindowMinimized = false; // Track if window is minimized for background crawling
  ...
  function renderPagination(totalPages, currentPage) {
    ...
    paginationList
      .querySelectorAll("a.url-pagination-number")
      .forEach((link) => {
        link.addEventListener("click", (e) => {
          ...
          if (
            ...
            document.getElementById("results-content").innerHTML =
              renderResults(paginatedResults, (currentPage - 1) * itemsPerPage);
    ...
    if (checkboxes.length > 0) {
      ...
      setTimeout(() => {
        ...
        if (exportSelectedBtn) {
          ...
          exportSelectedBtn.parentNode.replaceChild(
            newExportBtn,
            exportSelectedBtn,
          );
        ...
        const parseWithFiltersBtn = document.getElementById(
          "parse-with-filters-btn",
        );
        if (parseWithFiltersBtn) {
          const newParseBtn = parseWithFiltersBtn.cloneNode(true);
          parseWithFiltersBtn.parentNode.replaceChild(
            newParseBtn,
            parseWithFiltersBtn,
          );
  ...
  async function fetchCrawlResults(sessionId) {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/get-crawl-results/?session_id=${sessionId}`,
      );
  ...
  async function fetchCrawlStatus(sessionId) {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/get-crawl-status/?session_id=${sessionId}`,
      );
      const data = await response.json();
  ...
  function applyDisplayFilters() {
    ...
    filteredResults = allResults.filter((item) => {
      ...
      if (filters.linkType !== "all") {
        if (filters.linkType === "internal" && !item.is_internal)
          matches = false;
        if (filters.linkType === "external" && item.is_internal)
          matches = false;
      }
      ...
      if (filters.urlContains) {
        ...
        const search = filters.caseSensitive
          ? filters.urlContains
          : filters.urlContains.toLowerCase();
        if (!url.includes(search)) matches = false;
      }

      if (filters.urlExcludes) {
        const excludes = filters.urlExcludes
          .split(",")
          .map((ex) => ex.trim())
          .filter((ex) => ex);
        const url = filters.caseSensitive ? item.url : item.url.toLowerCase();
        for (const exclude of excludes) {
          const excludeTerm = filters.caseSensitive
            ? exclude
            : exclude.toLowerCase();
      ...
      if (filters.textContains) {
        const text = filters.caseSensitive
          ? item.text || ""
          : (item.text || "").toLowerCase();
        const search = filters.caseSensitive
          ? filters.textContains
          : filters.textContains.toLowerCase();
      ...
      if (filters.domain) {
        try {
          ...
          const domain = filters.caseSensitive
            ? urlObj.hostname
            : urlObj.hostname.toLowerCase();
          const searchDomain = filters.caseSensitive
            ? filters.domain
    ...
    isFilterActive = Object.values(filters).some(
    ...
    if (isPaused) {
      ...
      paginatedResults = paginateResults(
        resultsToShow,
        currentPage,
        itemsPerPage,
      );
  ...
  async function startFilteredParsing() {
    ...
    searchBtn.innerHTML =
      '<span class="spinner-border spinner-border-sm" role="status"></span> Crawling with Filters...';
    ...
    try {
      ...
      filterInfo.className =
        "alert alert-info alert-dismissible fade show mb-2";
  ...
  function init() {
    ...
    // Setup IPC listeners for window minimize/restore (for background crawling)
    if (typeof require !== "undefined") {
      try {
        const { ipcRenderer } = require("electron");

        ipcRenderer.on("window-minimized-throttle", () => {
          console.log(
            "🔍 Window minimized - continuing background crawl at full speed",
          );
          isWindowMinimized = true; // Set flag to prevent UI rendering but continue polling
          // Crawling continues at normal polling intervals (1s, 2s)
          // Data keeps being collected in background
        });

        ipcRenderer.on("window-restored-normal", () => {
          console.log("🔍 Window restored - UI updates resuming");
          isWindowMinimized = false; // Resume UI rendering
          // UI rendering resumes - all collected data will display
        });
      } catch (e) {
        console.warn("⚠️ Could not setup window listeners:", e);
      }
    }
    ...
    document
      ...
      ?.addEventListener("click", applyDisplayFilters);
    document
      .getElementById("clear-filters-btn")
      ?.addEventListener("click", clearFilters);
    document
      .getElementById("show-filters-btn")
    ...
    document.addEventListener("click", function (e) {
      ...
      if (
        e.target.id === "write-to-file-btn" ||
        e.target.closest("#write-to-file-btn")
      ) {
        const urlsToExport = isFilterActive ? filteredResults : allResults;
    ...
    searchBtn.addEventListener("click", async function (e) {
      ...
      const filters =
        typeof getCurrentFilters === "function" ? getCurrentFilters() : {};
      ...
      searchBtn.disabled = true;
      searchBtn.innerHTML =
        '<span class="spinner-border spinner-border-sm" role="status"></span renderLinkBatch()> Ползание...';
...
if (typeof window !== "undefined") {
  ...
  function initializeURLsModule() {
    ...
    if (window.__pages && window.__pages["urls"]) {
      ...
      console.log(
        "📋 AUTO-INIT: Available in window.__pages:",
